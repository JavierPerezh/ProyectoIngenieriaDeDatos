...
-- =============================================================================
-- MÓDULO 3 (LOTES) Y MÓDULO 4 (BODEGAS)
-- PROYECTO: SALINAS DEL CRAVO (INGENIERÍA DE DATOS)
-- =============================================================================

-- =============================================================================
-- 1. CONSULTAS BÁSICAS Y ESPECÍFICAS (FILTROS DE REQUISITOS DEL CLIENTE)
-- =============================================================================

-- -----------------------------------------------------------------------------
-- TABLA: bodega (Módulo 4)
-- -----------------------------------------------------------------------------
-- Consulta Básica: Inventario general de infraestructura de almacenamiento
SELECT id_bodega, nombre_bodega, direccion_fisica, capacidad_maxima_bultos, estado_activo FROM bodega;

-- Específica 1: Filtrar bodegas con capacidad limitada o menor a 1000 bultos
SELECT * FROM bodega WHERE capacidad_maxima_bultos < 1000;

-- Específica 2: Buscar una bodega estratégica por su nombre exacto en el sistema
SELECT * FROM bodega WHERE nombre_bodega = 'Bodega Principal Sogamoso';

-- Específica 3: Contar el número total de centros de almacenamiento (bodegas) operativos
SELECT COUNT(*) AS total_bodegas_activas FROM bodega WHERE estado_activo = 1;

-- Específica 4: Ranking de bodegas ordenadas de mayor a menor según su espacio de diseño
SELECT * FROM bodega ORDER BY capacidad_maxima_bultos DESC;

-- Específica 5: Buscar bodegas ubicadas específicamente en el municipio de Sogamoso
SELECT * FROM bodega WHERE direccion_fisica LIKE '%Sogamoso%';

-- Específica 6: Filtrar bodegas medianas aptas para almacenamiento selectivo (Capacidad entre 500 y 1500)
SELECT * FROM bodega WHERE capacidad_maxima_bultos BETWEEN 500 AND 1500;

-- Específica 7: Monitorear la bodega con menor infraestructura (Capacidad más baja)
SELECT nombre_bodega, direccion_fisica, capacidad_maxima_bultos FROM bodega ORDER BY capacidad_maxima_bultos ASC LIMIT 1;


-- -----------------------------------------------------------------------------
-- TABLA: entrada / Lotes de Producción (Módulo 3)
-- -----------------------------------------------------------------------------

-- Consulta Básica: Historial de todos los lotes de sal mineralizada que han ingresado
SELECT id_entrada, id_stock, id_bodega, cantidad_ingresada, numero_lote, fecha_entrada FROM entrada;

-- Específica 1: Trazabilidad completa de un registro de entrada mediante su identificador único
SELECT * FROM entrada WHERE id_entrada = 1;

-- Específica 2: Control de producción y entradas en una fecha operativa puntual (Año 2026)
SELECT * FROM entrada WHERE DATE(fecha_entrada) = '2026-04-20';

-- Específica 3: Monitorear lotes de alta densidad/volumen (Ingresos mayores a 200 bultos de sal)
SELECT * FROM entrada WHERE cantidad_ingresada > 200;

-- Específica 4: Reporte de operaciones de entrada por un tipo de ingreso específico (Compras)
SELECT * FROM entrada WHERE tipo_ingreso = 'compra';

-- Específica 5: Consolidado histórico de volumen total de bultos de sal mineralizada ingresados
SELECT SUM(cantidad_ingresada) AS bultos_totales_ingresados FROM entrada;

-- Específica 6: Identificar el lote con el mayor volumen de bultos registrado en una sola entrada
SELECT * FROM entrada ORDER BY cantidad_ingresada DESC LIMIT 1;

-- Específica 7: Auditoría de entradas ordenadas cronológicamente desde la más reciente
SELECT * FROM entrada ORDER BY fecha_entrada DESC;


-- =============================================================================
-- 2. MODIFICACIONES Y ELIMINACIÓN (OPERACIONES DE MANTENIMIENTO)
-- =============================================================================

-- Modificación 1: RQF de expansión - Aumentar la capacidad de almacenamiento por reestructuración física
UPDATE bodega SET capacidad_maxima_bultos = 2500 WHERE id_bodega = 1;

-- Modificación 2: Actualizar la dirección física o nomenclatura de una bodega específica
UPDATE bodega SET direccion_fisica = 'Calle 14 #12-35 Zona Industrial Sogamoso' WHERE id_bodega = 1;

-- Modificación 3: Corrección de auditoría - Modificar la cantidad ingresada de una entrada por descuadre
UPDATE entrada SET cantidad_ingresada = 150 WHERE id_entrada = 1;

-- Modificación 4: Actualizar las observaciones de control de un lote de entrada específico
UPDATE entrada SET observaciones = 'Verificado con el remito del proveedor' WHERE id_entrada = 2;

-- Modificación 5: Modificar o asignar el número de lote de trazabilidad de una entrada
UPDATE entrada SET numero_lote = 'LOT-2026-V3' WHERE id_entrada = 1;

-- Eliminación: Anular un registro de entrada erróneo (Mantenimiento logístico guiado)
DELETE FROM entrada WHERE id_entrada = 999; 


-- =============================================================================
-- 3. CONSULTAS MULTITABLA Y SUBCONSULTAS (MÓDULOS ENLAZADOS)
-- =============================================================================

-- Consulta Multitabla 1: Reporte maestro de trazabilidad de entradas (Une Entrada, Stock, Producto y Bodega)
-- Responde al RQF de ver los detalles de los productos que ingresan y a qué bodegas físicas se asignan.
SELECT 
    e.id_entrada AS Codigo_Entrada,
    e.numero_lote AS Numero_Lote,
    p.nombre_sal_mineralizada AS Tipo_Sal,
    e.cantidad_ingresada AS Bultos_Ingresados,
    e.fecha_entrada AS Fecha_Registro,
    b.nombre_bodega AS Bodega_Destino,
    e.tipo_ingreso AS Motivo_Ingreso
FROM entrada e
INNER JOIN stock s ON e.id_stock = s.id_stock
INNER JOIN producto p ON s.id_producto = p.id_producto
INNER JOIN bodega b ON e.id_bodega = b.id_bodega;

-- Consulta Multitabla 2: Consolidado de ocupación real en bultos de las bodegas vs su capacidad de diseño
-- Muestra el stock total acumulado de todos los productos por cada bodega frente a su límite máximo de bultos.
SELECT 
    b.id_bodega,
    b.nombre_bodega AS Bodega,
    b.capacidad_maxima_bultos AS Capacidad_Maxima,
    COALESCE(SUM(s.cantidad_actual), 0) AS Bultos_Almacenados_Actuales,
    (b.capacidad_maxima_bultos - COALESCE(SUM(s.cantidad_actual), 0)) AS Espacio_Disponible_Bultos
FROM bodega b
LEFT JOIN stock s ON b.id_bodega = s.id_bodega
GROUP BY b.id_bodega, b.nombre_bodega, b.capacidad_maxima_bultos;


-- Subconsulta 1: Filtrar registros de entradas cuya cantidad supere el promedio global de ingresos
SELECT id_entrada, numero_lote, cantidad_ingresada, fecha_entrada 
FROM entrada 
WHERE cantidad_ingresada > (SELECT AVG(cantidad_ingresada) FROM entrada);

-- Subconsulta 2: Identificar bodegas que almacenan productos que pertenecen a la categoría 'Sal Mineralizada 12%'
SELECT * FROM bodega 
WHERE id_bodega IN (
    ...
    WHERE id_producto IN (
        SELECT id_producto 
        FROM producto 
        WHERE id_categoria = (SELECT id_categoria FROM categoria WHERE nombre_categoria LIKE '%12%%' LIMIT 1)
    )
);


-- =============================================================================
-- 4. VISTAS Y PROCEDIMIENTOS ALMACENADOS (AUTOMATIZACIÓN LOGÍSTICA)
-- =============================================================================

-- Limpieza preventiva de objetos de base de datos
DROP VIEW IF EXISTS vista_alertas_capacidad_bodegas;
DROP PROCEDURE IF EXISTS sp_insertar_entrada_segura;
DROP PROCEDURE IF EXISTS sp_consultar_entradas_por_producto;

-- -----------------------------------------------------------------------------
-- VISTA: Monitor de Alertas de Capacidad Crítica y Ocupación de Bodegas
-- RQF Automatizado: Permite conocer el estado porcentual de llenado de cada bodega en tiempo real.
-- -----------------------------------------------------------------------------
CREATE VIEW vista_alertas_capacidad_bodegas AS
SELECT 
    b.id_bodega,
    b.nombre_bodega AS bodega,
    b.capacidad_maxima_bultos,
    COALESCE(SUM(s.cantidad_actual), 0) AS bultos_en_stock,
    CASE 
        WHEN COALESCE(SUM(s.cantidad_actual), 0) > b.capacidad_maxima_bultos THEN 'ALERTA: SOBRECUPO CRÍTICO'
        WHEN COALESCE(SUM(s.cantidad_actual), 0) = b.capacidad_maxima_bultos THEN 'ADVERTENCIA: BODEGA AL LÍMITE (100%)'
        WHEN COALESCE(SUM(s.cantidad_actual), 0) >= (b.capacidad_maxima_bultos * 0.85) THEN 'PRECAUCIÓN: BODEGA AL 85% O MÁS'
        ELSE 'ESTADO: ÓPTIMO / ESPACIO DISPONIBLE'
    END AS estado_alerta
FROM bodega b
LEFT JOIN stock s ON b.id_bodega = s.id_bodega
GROUP BY b.id_bodega, b.nombre_bodega, b.capacidad_maxima_bultos;

SELECT * FROM vista_alertas_capacidad_bodegas;
SELECT * FROM vista_alertas_capacidad_bodegas WHERE estado_alerta LIKE '%ALERTA%' OR estado_alerta LIKE '%PRECAUCIÓN%';

-- -----------------------------------------------------------------------------
-- PROCEDIMIENTO ALMACENADO 1: Registro seguro de Entradas con validación logístico-comercial
-- RQF Automatizado: Controla que no ingresen bultos vacíos o en negativo al sistema.
-- -----------------------------------------------------------------------------
DELIMITER //

CREATE PROCEDURE sp_insertar_entrada_segura(
    IN p_id_stock INT,
    IN p_id_usuario INT,
    IN p_id_bodega INT,
    IN p_id_movimiento INT,
    IN p_cantidad_ingresada INT,
    IN p_tipo_ingreso VARCHAR(80),
    IN p_numero_lote VARCHAR(50),
    IN p_observaciones TEXT
)
BEGIN
    -- Regla de Negocio: No se permiten ingresos vacíos o inconsistentes
    IF p_cantidad_ingresada <= 0 THEN
        SIGNAL SQLSTATE '45000' 
        SET MESSAGE_TEXT = 'Error Logístico: La cantidad ingresada del lote debe ser estrictamente mayor a cero.';
    ELSE
        INSERT INTO entrada (
            id_stock, id_usuario, id_bodega, id_movimiento, 
            cantidad_ingresada, tipo_ingreso, numero_lote, observaciones, fecha_entrada
        )
        VALUES (
            p_id_stock, p_id_usuario, p_id_bodega, p_id_movimiento, 
            p_cantidad_ingresada, p_tipo_ingreso, p_numero_lote, p_observaciones, NOW()
        );
    END IF;
END //
...
CALL sp_insertar_entrada_segura(1, 1, 1, 1, 120, 'produccion', 'LOTE-SOG-04', 'Ingreso de lote aprobado por control de calidad.');


-- -----------------------------------------------------------------------------
-- PROCEDIMIENTO ALMACENADO 2: Historial dinámico de Entradas/Lotes por Producto
-- RQF Automatizado: Permite al cliente rastrear dinámicamente qué lotes y bultos han ingresado de un SKU específico.
-- -----------------------------------------------------------------------------
DELIMITER //

CREATE PROCEDURE sp_consultar_entradas_por_producto(
    IN p_id_producto INT
)
BEGIN
    SELECT 
        e.id_entrada AS codigo_ingreso,
        e.numero_lote AS lote_produccion,
        b.nombre_bodega AS bodega_destino,
        e.cantidad_ingresada AS bultos_ingresados,
        e.fecha_entrada AS fecha_ingreso,
        e.tipo_ingreso AS modalidad
    FROM entrada e
    INNER JOIN stock s ON e.id_stock = s.id_stock
    INNER JOIN bodega b ON e.id_bodega = b.id_bodega
...
-- rastrear todos los lotes de producción que se han guardado del producto con ID = 1
CALL sp_consultar_entradas_por_producto(1);

-- rastrear el historial del producto con ID = 2
CALL sp_consultar_entradas_por_producto(2);


DELIMITER //

CREATE TRIGGER tr_actualizar_stock_por_entrada
AFTER INSERT ON entrada
FOR EACH ROW
BEGIN
    UPDATE stock 
    SET cantidad_actual = cantidad_actual + NEW.cantidad_ingresada
    WHERE id_stock = NEW.id_stock;
END //

DELIMITER ;

-- 1. el stock actual de un registro (Ejemplo: id_stock = 1)
SELECT id_stock, id_producto, id_bodega, cantidad_actual FROM stock WHERE id_stock = 1;

-- 2. Insertamos una nueva entrada amarrada a ese id_stock (Asegúrate de usar IDs válidos de tu base de datos)
INSERT INTO entrada (id_stock, id_usuario, id_bodega, id_movimiento, cantidad_ingresada, tipo_ingreso, numero_lote, observaciones, fecha_entrada)
VALUES (1, 1, 1, 150, 80, 'produccion', 'LOT-TEST-01', 'Ingreso de prueba para disparador', NOW());

-- 3. aumentó automáticamente en 80 unidades sin hacer un UPDATE manual.
SELECT id_stock, id_producto, id_bodega, cantidad_actual FROM stock WHERE id_stock = 1;


-- -----------------------------------------------------------------------------
-- TRIGGER 2: tr_validar_capacidad_bodega (Módulo 4 - Bodegas / Almacenamiento)
-- RQF Automatizado: Prevenir pérdidas e infraestructura sobrepasada.
-- Antes de actualizar las cantidades en 'stock', verifica que la suma total de bultos
-- en esa bodega no supere la 'capacidad_maxima_bultos' de la tabla 'bodega'.
-- -----------------------------------------------------------------------------
DELIMITER //

CREATE TRIGGER tr_validar_capacidad_bodega
...
BEGIN
    ...
    -- 1. Obtener la capacidad máxima de diseño de la bodega involucrada
    SELECT capacidad_maxima_bultos INTO v_capacidad_max 
    FROM bodega 
    WHERE id_bodega = NEW.id_bodega;
    
    -- 2. Calcular cuántos bultos hay en total en esa bodega (excluyendo el valor viejo de este registro)
    SELECT COALESCE(SUM(cantidad_actual), 0) INTO v_ocupacion_actual
    FROM stock
    WHERE id_bodega = NEW.id_bodega AND id_stock <> NEW.id_stock;
    
    -- 3. Validar si el nuevo valor proyectado sobrepasa el límite físico de la bodega
    IF (v_ocupacion_actual + NEW.cantidad_actual) > v_capacidad_max THEN
        SIGNAL SQLSTATE '45000'
        SET MESSAGE_TEXT = 'LOGÍSTICA ERROR: Operación cancelada. El volumen de bultos supera la capacidad máxima permitida para esta bodega.';
    END IF;
END //

DELIMITER ;
-- 1. Consultemos la capacidad máxima de una bodega (Ejemplo: id_bodega = 2) y su stock real actual
SELECT id_bodega, nombre_bodega, capacidad_maxima_bultos FROM bodega WHERE id_bodega = 2;
SELECT id_stock, cantidad_actual FROM stock WHERE id_bodega = 2;