...
-- Consultas básicas: Tabla Categoria

-- ===============================================================

SELECT * FROM categoria;
 
-- Consulta 1: Categorías ordenadas por porcentaje de fósforo descendente
SELECT id_categoria, nombre_categoria, porcentaje_fosforo
FROM categoria
ORDER BY porcentaje_fosforo DESC;

-- Consulta 2: Categorías con porcentaje de fósforo mayor al 6%
SELECT nombre_categoria, porcentaje_fosforo
FROM categoria
WHERE porcentaje_fosforo > 6.0
ORDER BY porcentaje_fosforo DESC;

-- Consulta 3: Número de productos por categoría
...
-- Consultas básicas: Tabla Producto

-- ===============================================================
-- Consulta 1: Productos activos
SELECT id_producto, nombre_sal_mineralizada, unidad_medida
FROM producto
WHERE descontinuado = 0
ORDER BY id_producto;

-- Consulta 2: Productos con presentación a granel
SELECT id_producto, nombre_sal_mineralizada, peso_bulto_kg
FROM producto
WHERE unidad_medida = 'granel';

-- Consulta 3: Productos ordenados por peso de mayor a menor
SELECT nombre_sal_mineralizada, peso_bulto_kg, unidad_medida
FROM producto
ORDER BY peso_bulto_kg DESC, nombre_sal_mineralizada ASC;

-- ===============================================================

-- Consultas básicas: Tabla Cliente

-- ===============================================================

SELECT * FROM cliente;

-- Consulta 1: Clientes ordenados por nombre
SELECT id_cliente, nombre_cliente, nit_cedula, telefono, tipo
FROM cliente
ORDER BY nombre_cliente;

-- Consulta 2: Clientes activos 
SELECT id_cliente, nombre_cliente, nit_cedula, telefono, tipo
FROM cliente
WHERE estado_activo = 1;

-- Consulta 3: Clientes ubicados en Sogamoso
SELECT nombre_cliente, direccion_entrega
FROM cliente
WHERE direccion_entrega LIKE '%Sogamoso%';

-- Consulta 4: Conteo de clientes por estado (activo / inactivo)
SELECT
    CASE estado_activo WHEN 1 THEN 'Activo' ELSE 'Inactivo' END AS estado,
    COUNT(*) AS total
FROM cliente
GROUP BY estado_activo;

-- Consulta 5: Búsqueda de un cliente por NIT o cédula
SELECT id_cliente, nombre_cliente, nit_cedula, telefono, direccion_entrega, tipo, estado_activo
FROM cliente
WHERE nit_cedula = '900112233-1';

-- Consulta 6: Historial de compras de un cliente — bultos adquiridos
--            y producto, ordenados cronológicamente  (RQF009)
SELECT
    c.nombre_cliente,
    p.nombre_sal_mineralizada,
    mi.cantidad_bultos,
    mi.timestamp_mov AS fecha_compra
FROM cliente c
JOIN movimiento_inventario mi ON mi.id_cliente = c.id_cliente
JOIN stock s                  ON s.id_stock    = mi.id_stock
JOIN producto p               ON p.id_producto = s.id_producto
WHERE mi.tipo_mov = 'SALIDA'
ORDER BY c.nombre_cliente, mi.timestamp_mov;
...
CHANGE descripcion_uso descripcion_categoria VARCHAR(255) NOT NULL;
 
-- Modificación DDL 2:
ALTER TABLE cliente
ADD COLUMN correo_electronico VARCHAR(120) NULL
AFTER telefono;
    
-- Modificación DML 1: Actualizar teléfono de un cliente
UPDATE cliente
SET telefono = '3200001111'
WHERE nit_cedula = '900112233-1';
 
-- Modificación DML 2: Actualizar dirección de entrega de un cliente
UPDATE cliente
SET direccion_entrega = 'Calle 5 # 10-99, Sogamoso, Boyacá'
WHERE nit_cedula = '900223344-2';
 
-- Modificación DML 3: Inactivar un cliente que ya no compra
UPDATE cliente
SET estado_activo = 0
WHERE nit_cedula = '905334455-8';

UPDATE cliente SET correo_electronico = 'elpotrero@vet.com'      WHERE id_cliente = 1;
UPDATE cliente SET correo_electronico = 'losandes@agrovet.com'   WHERE id_cliente = 2;
UPDATE cliente SET correo_electronico = 'boyacavet@clinica.com'  WHERE id_cliente = 3;
UPDATE cliente SET correo_electronico = 'campoverde@vet.com'     WHERE id_cliente = 4;
UPDATE cliente SET correo_electronico = 'lasabana@agropec.com'   WHERE id_cliente = 5;
...
-- ============================================================
-- 6. CONSULTAS MULTITABLA (JOINs) y SUBCONSULTAS
-- ============================================================

	-- ============================================================
	-- 6.1 CONSULTAS MULTITABLA (JOINs)
    -- ============================================================
  
  -- JOIN 1: Catálogo completo — producto con su categoría  (RQF011 / RQF013)
SELECT
    c.nombre_categoria,
    ...
    p.peso_bulto_kg,
    p.unidad_medida,
    p.paquetes_por_bulto,
    CASE p.descontinuado WHEN 1 THEN 'Descontinuado' ELSE 'Activo' END AS estado
FROM categoria c
JOIN producto p ON p.id_categoria = c.id_categoria
ORDER BY c.porcentaje_fosforo DESC, p.unidad_medida;

-- JOIN 2: Qué productos ha comprado cada cliente (detalle de compras)
SELECT
    c.nombre_cliente,
    p.nombre_sal_mineralizada,
    cat.nombre_categoria,
    SUM(mi.cantidad_bultos) AS bultos_comprados
FROM cliente c
JOIN movimiento_inventario mi ON mi.id_cliente  = c.id_cliente
                              AND mi.tipo_mov   = 'SALIDA'
JOIN stock    s               ON s.id_stock     = mi.id_stock
JOIN producto p               ON p.id_producto  = s.id_producto
JOIN categoria cat            ON cat.id_categoria = p.id_categoria
GROUP BY c.id_cliente, c.nombre_cliente, p.id_producto,
         p.nombre_sal_mineralizada, cat.nombre_categoria
ORDER BY c.nombre_cliente, bultos_comprados DESC;

-- JOIN 3: Clientes con su historial de compras resumido
SELECT
    c.nombre_cliente,
    c.nit_cedula,
    c.tipo,
    COUNT(mi.id_movimiento)   AS total_pedidos,
    SUM(mi.cantidad_bultos)   AS total_bultos_comprados,
    MAX(mi.timestamp_mov)     AS ultima_compra
FROM cliente c
JOIN movimiento_inventario mi ON mi.id_cliente = c.id_cliente
                              AND mi.tipo_mov  = 'SALIDA'
GROUP BY c.id_cliente, c.nombre_cliente, c.nit_cedula, c.tipo
...
-- Subconsulta 1: Productos que NUNCA han sido vendidos
SELECT p.id_producto, p.nombre_sal_mineralizada, p.unidad_medida
FROM producto p
...
-- Subconsulta 2: Categoría más vendida
SELECT c.nombre_categoria, c.porcentaje_fosforo
FROM categoria c
WHERE c.id_categoria = (
    ...
    ORDER BY SUM(mi.cantidad_bultos) DESC
    LIMIT 1
);
...
-- Vista 1: catalogo_productos_activos
CREATE OR REPLACE VIEW catalogo_productos_activos AS
SELECT
    p.id_producto,
    cat.nombre_categoria,
    cat.porcentaje_fosforo,
    p.nombre_sal_mineralizada,
    p.peso_bulto_kg,
    p.unidad_medida,
    p.paquetes_por_bulto
...
ORDER BY cat.porcentaje_fosforo DESC, p.unidad_medida;
 
-- Uso de la vista:
SELECT * FROM catalogo_productos_activos;
SELECT * FROM catalogo_productos_activos WHERE nombre_categoria LIKE '%12%';

-- Vista 2: resumen_clientes_activos
...
SELECT
    ...
    c.direccion_entrega,
    c.tipo,
    COUNT(mi.id_movimiento)          AS total_compras,
    COALESCE(SUM(mi.cantidad_bultos), 0) AS total_bultos_comprados,
    MAX(mi.timestamp_mov)            AS ultima_compra
FROM cliente c
LEFT JOIN movimiento_inventario mi ON mi.id_cliente = c.id_cliente
                                   AND mi.tipo_mov  = 'SALIDA'
WHERE c.estado_activo = 1
GROUP BY c.id_cliente, c.nombre_cliente, c.nit_cedula,
         c.telefono, c.correo_electronico, c.direccion_entrega, c.tipo;
 
-- Uso de la vista:
SELECT * FROM resumen_clientes_activos ORDER BY total_bultos_comprados DESC;
SELECT * FROM resumen_clientes_activos WHERE nombre_cliente LIKE '%Sogamoso%';
...
CREATE PROCEDURE registrar_cliente(
    IN  p_nit_cedula        VARCHAR(20),
    IN  p_nombre_cliente    VARCHAR(150),
    IN  p_telefono          VARCHAR(20),
    IN  p_correo            VARCHAR(120),
    IN  p_direccion         VARCHAR(255),
    IN  p_tipo              ENUM('VETERINARIA','GANADERO'),
    OUT p_resultado         VARCHAR(100)
)
BEGIN
    -- Verificar si el NIT ya está registrado
    IF EXISTS (SELECT 1 FROM cliente WHERE nit_cedula = p_nit_cedula) THEN
        SET p_resultado = 'ERROR: El NIT/cédula ya se encuentra registrado.';
    ELSE
        INSERT INTO cliente (nit_cedula, nombre_cliente, telefono, correo_electronico,
                             direccion_entrega, tipo, estado_activo)
        VALUES (p_nit_cedula, p_nombre_cliente, p_telefono, p_correo,
                p_direccion, p_tipo, 1);
 
        SET p_resultado = CONCAT('OK: Cliente registrado con ID ', LAST_INSERT_ID(), '.');
    END IF;
END //
...
CALL registrar_cliente(
    '910000001-1',
    'Veterinaria Nueva Esperanza',
    '3101234567',
    'nuevaesperanza@vet.com',
    'Calle 1 # 1-10, Sogamoso',
    'VETERINARIA',
    @resultado
);
SELECT @resultado AS resultado;

-- Procedimiento 2: Actualizar Cliente
DELIMITER //
 
CREATE PROCEDURE actualizar_contacto_cliente(
    IN  p_id_cliente        INT,
    IN  p_nuevo_telefono    VARCHAR(20),
    IN  p_nuevo_correo      VARCHAR(120),
    IN  p_nueva_direccion   VARCHAR(255),
    OUT p_resultado         VARCHAR(100)
)
BEGIN
    -- Verificar que el cliente exista y esté activo
    IF NOT EXISTS (SELECT 1 FROM cliente WHERE id_cliente = p_id_cliente AND estado_activo = 1) THEN
        SET p_resultado = 'ERROR: Cliente no encontrado o inactivo.';
    ELSE
        UPDATE cliente
        SET
            telefono          = COALESCE(p_nuevo_telefono,  telefono),
            correo_electronico = COALESCE(p_nuevo_correo,   correo_electronico),
            direccion_entrega = COALESCE(p_nueva_direccion, direccion_entrega)
        WHERE id_cliente = p_id_cliente;
 
        SET p_resultado = CONCAT('OK: Contacto del cliente ID ', p_id_cliente, ' actualizado.');
    END IF;
END //
...
CALL actualizar_contacto_cliente(
    ...
    '3150005555',
    'elparamo@vet.com',
    'Carrera 12 # 3-55, Paipa, Boyacá',
    @resultado
);
SELECT @resultado AS resultado;
 
-- Verificar procedimiento
SELECT id_cliente, nombre_cliente, telefono, correo_electronico, direccion_entrega
FROM cliente
WHERE id_cliente = 6;

DELIMITER //

-- Procedimiento 3: Registrar Producto
...
BEGIN
    IF EXISTS (
        SELECT 1 
        FROM producto 
        WHERE nombre_sal_mineralizada = p_nombre
    ) THEN
    ...
    ELSE
        ...
        SET p_resultado = CONCAT('OK: Producto registrado con ID ', LAST_INSERT_ID());
    END IF;
END //
...
CALL registrar_producto(
    1,
    'Sal Mineralizada 12% — Prueba 25kg',
    25.00,
    '25kg',
    1,
    @resultado
...
SELECT *
FROM producto
WHERE nombre_sal_mineralizada LIKE '%Prueba 25kg%';

-- Procedimiento 4: Actualizar Producto
DELIMITER //

CREATE PROCEDURE actualizar_producto(
    IN  p_id_producto     INT,
    IN  p_nuevo_nombre    VARCHAR(150),
    IN  p_nuevo_peso      DECIMAL(6,2),
    IN  p_nueva_unidad    VARCHAR(50),
    IN  p_nuevos_paquetes INT,
    OUT p_resultado       VARCHAR(100)
)
BEGIN
    IF NOT EXISTS (SELECT 1 FROM producto WHERE id_producto = p_id_producto) THEN
        SET p_resultado = 'ERROR: Producto no existe.';
    ELSE
        UPDATE producto
        SET
            nombre_sal_mineralizada = COALESCE(p_nuevo_nombre, nombre_sal_mineralizada),
            peso_bulto_kg           = COALESCE(p_nuevo_peso, peso_bulto_kg),
            unidad_medida           = COALESCE(p_nueva_unidad, unidad_medida),
            paquetes_por_bulto      = COALESCE(p_nuevos_paquetes, paquetes_por_bulto)
        WHERE id_producto = p_id_producto;

        SET p_resultado = CONCAT('OK: Producto ', p_id_producto, ' actualizado.');
    END IF;
END //

DELIMITER ;

-- Prueba del procedimiento 4:
CALL actualizar_producto(
    1,
    'Sal Mineralizada 12% — EDITADO',
    55.00,
    NULL,
    NULL,
    @resultado
...
-- Procedimiento 5: Descontinuar producto
DELIMITER //

CREATE PROCEDURE descontinuar_producto(
    IN p_id_producto INT,
    OUT p_resultado  VARCHAR(100)
)
BEGIN
    ...
    ELSE
        ...
        SET p_resultado = CONCAT('OK: Producto ', p_id_producto, ' marcado como inactivo.');
    END IF;
END //

DELIMITER ;

-- Prueba del procedimiento 5:
CALL descontinuar_producto(
    2,
    @resultado
);
SELECT @resultado AS resultado;
...
-- Verificar procedimiento
SELECT id_producto, nombre_sal_mineralizada, descontinuado
FROM producto
WHERE id_producto = 2;


-- Procedimiento 6: Inactivar Cliente
DELIMITER //
...
BEGIN
    IF NOT EXISTS (SELECT 1 FROM cliente WHERE id_cliente = p_id_cliente) THEN
        SET p_resultado = 'ERROR: Cliente no existe.';
    ELSE
        UPDATE cliente
        SET estado_activo = 0
        WHERE id_cliente = p_id_cliente;

        SET p_resultado = CONCAT('OK: Cliente ', p_id_cliente, ' inactivado.');
    END IF;
END //

DELIMITER ;

-- Prueba del procedimiento 6
CALL inactivar_cliente(
    1,
    @resultado
);
SELECT @resultado AS resultado;
...
-- Verificar procedimiento
SELECT id_cliente, nombre_cliente, estado_activo
FROM cliente
WHERE id_cliente = 1;